...
def CreateAntlrParser(
    ...
    parser_newline_string: str | bool | None = None,  # noqa: FBT001
) -> AntlrParser:
    """Create an `AntlrParser` that uses the given code generated by ANTLR."""

    indent_token_type = getattr(antlr_parser, "INDENT", None)
    dedent_token_type = getattr(antlr_parser, "DEDENT", None)

    if parser_newline_string is None and indent_token_type is not None and dedent_token_type is not None:
        parser_newline_string = "newLine"
    elif isinstance(parser_newline_string, bool):
            ...
            with ExitStack(OnExit):
                try:
                    ...
                    visitor = self._create_visitor_func(
                        ...
                        indent_token_type=self._indent_token_type,
                        dedent_token_type=self._dedent_token_type,
                        parser_newline_string=self._parser_newline_string,
                    )
